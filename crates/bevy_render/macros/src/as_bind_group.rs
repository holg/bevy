...
pub fn derive_as_bind_group(ast: syn::DeriveInput) -> Result<TokenStream> {
    ...
    // Tracks whether any `BindlessResourceType::Buffer` resources are used
    // (i.e. `#[uniform(..., binding_array(...))]` or `#[storage(..., binding_array(...))]`).
    // When false, the material only uses data buffers, textures, and samplers,
    // so `BUFFER_BINDING_ARRAY` is not required (enabling Metal support).
    ...
    let (bindless_slot_count, actual_bindless_slot_count_declaration, bindless_descriptor_syntax) =
        match attr_bindless_count {
            Some(ref bindless_count) => {
                // Only require BUFFER_BINDING_ARRAY when the material
                // actually uses buffer binding arrays (i.e.
                // `#[uniform(..., binding_array(...))]` or
                // `#[storage(..., binding_array(...))]`). Materials that
                // only use `#[data(...)]`, textures, and samplers (like
                // `StandardMaterial`) don't need buffer binding arrays
                // and can use bindless on platforms like Metal that
                // support texture/sampler binding arrays but not buffer
                // binding arrays.
