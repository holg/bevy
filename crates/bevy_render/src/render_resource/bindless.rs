...
///
/// `bindless_index_table_length` specifies the total number of bindless
/// resources. `bindless_slab_resource_limit` specifies the resolved
/// [`BindlessSlabResourceLimit`] value. `used_resource_types` specifies
/// which resource types the material actually uses; only binding arrays for
/// those types are created, which reduces the number of Metal argument
/// buffer slots consumed.
pub fn create_bindless_bind_group_layout_entries(
    ...
    // Only create binding arrays for resource types that this material
    // actually uses. This is important on Metal where each binding array
    // consumes a buffer slot and the total is limited to 31.
